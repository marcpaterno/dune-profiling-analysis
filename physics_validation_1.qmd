...
Below are two channels, from a single event, showing the time profile. Channel 91 seems to contain only noise, and channel 92 seems to contain two signal pulses:
...
orig_ticks |>
  filter(nphot > 20) |>
  group_by(run,subrun,event,channel) |>
