...
* I have been working on improving the speed of the `PDFastSimPAR` module.
  This module does a fast simulation of propagation of the photons created from `SimEnergyDeposits`, "Semi-Analytical model", which stores the visibilities of each optical channel with respect to each optical voxel in the TPC volume, to avoid propagating single photons using Geant.
...
`key` = tick (`int`); `value` = nphotons (`int`)
:::
:::{.column width=35%}
* For `SPL`, the size of each node is 36 bytes (including padding); of this 8 bytes are the data `key` and `value` (more than 75% of the space is overhead).
